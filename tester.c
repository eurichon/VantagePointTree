...
int verifyTree(vptree *T, double *vp, node **stack, double md, int isInner,
  ...
  if (isInner && getMD(T) > 2*md) {
	  printf("inner, radius must is smaller than parent's diameter");
	  return 0;
  }
  ...
  printf("Distance validation: %i cause %lf, %lf and median is: %lf\n",isValid,vp[0],getVP(T)[0],md);
  // recurse if not leaf
  if (!isLeaf) {
	...
	printf("validation: isvalid=%i, isValid_in=%i, isvalid_out=%i\n",isValid,isValidInn,isValidOut);
    ...
    printf("validation: isvalid=%i, isValidAncestor=%i\n",isValid,isValidAncestor);
  }
